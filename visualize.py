...
def plot_mode_shapes_continuous(n_elem, L_total, mode_shapes_classical,
    ...
    x_all = np.linspace(0, L_total, 2 * n_elem + 1)[1::2]  # transverse DOF locs
    x_all = np.array([i * L_total / (2 * n_elem) for i in range(1, 2 * n_elem + 1, 2)])

    for mode_idx in range(min(n_modes, len(axes))):
        ax = axes[mode_idx]

        # Classical mode shape
        u_classical = np.zeros(2 * n_elem + 2)
        # Map free DOFs back to full DOFs
        full_dofs = list(range(2 * n_elem + 2))
        free_mask = [d not in [0, 2 * n_elem] for d in full_dofs]  # pinned BC
        u_full = np.zeros(2 * n_elem + 2)
        free_iter = 0
        for i, is_free in enumerate(free_mask):
            if is_free:
                u_full[i] = mode_shapes_classical[free_iter, mode_idx]
                free_iter += 1
        ...
        u_plot_classical = []
        u_plot_vqe = []
        for i in range(0, 2 * n_elem + 2, 2):
            x_plot.append(i / 2 * (L_total / n_elem))
            u_plot_classical.append(u_full[i])

        # Normalize for plotting
        u_plot_classical = np.array(u_plot_classical)
        if np.max(np.abs(u_plot_classical)) > 0:
            u_plot_classical = u_plot_classical / np.max(np.abs(u_plot_classical))

        ax.plot(x_plot, u_plot_classical, 'o-', color=RED,
                linewidth=2, markersize=8, label='Classical FEA')

        # VQE mode shape if available
        if mode_shapes_vqe and mode_idx < len(mode_shapes_vqe):
            u_vqe = mode_shapes_vqe[mode_idx]
            # Map to physical DOFs (tricky - depends on encoding)
            if len(u_vqe) <= len(x_plot):
                u_vqe_mapped = np.zeros(len(x_plot))
                # Simple mapping for small systems
                for i in range(min(len(u_vqe), len(x_plot))):
                    u_vqe_mapped[i] = u_vqe[min(i, len(u_vqe)-1)]
                if np.max(np.abs(u_vqe_mapped)) > 0:
                    u_vqe_mapped = u_vqe_mapped / np.max(np.abs(u_vqe_mapped))
                ax.plot(x_plot[:len(u_vqe_mapped)], u_vqe_mapped, 's--',
                       color=BLUE, linewidth=2, markersize=8, label='VQE')

        # Analytical if available
        if analytical_params and mode_idx == 0:
            L_ana = analytical_params.get('L', L_total)
            x_ana = np.linspace(0, L_ana, 200)
            # Simply supported beam mode shape: sin(n pi x / L)
            u_ana = np.sin((mode_idx + 1) * np.pi * x_ana / L_ana)
